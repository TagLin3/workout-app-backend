...
const errorHandler = (err, req, res, next) => {
  if (err.name === "ValidationError") {
    ...
    res.status(401).set("WWW-Authenticate", "Bearer").json({ error: "Invalid authorization token" });
  } else if (err.name === "TokenExpiredError") {
    res.status(401).set("WWW-Authenticate", "Bearer").json({ error: "Authorization token expired" });
  } else {
